...
cam = srcampy.Camera()
cam1 = cam.open_cam(1, -1, 30,1920,1080)  # kuan gao
im_show = ImageShow()
  
#视频流循环帧
while True:
    frame = cam.get_img(2, 1920, 1080) 
    # print(type(frame),len(frame))
    frame_array = np.frombuffer(frame, dtype=np.uint8).reshape(1620, 1920) 
    print(frame_array.shape)
    ori_bgr_cam = cv2.cvtColor(frame_array, cv2.COLOR_YUV420SP2RGB)
    print(ori_bgr_cam.shape)
    im_show.show(ori_bgr_cam)
