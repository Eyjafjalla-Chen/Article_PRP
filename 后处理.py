...
def process_frame(model_output, model_hw_shape, origin_image=None):
    bboxes = postprocess(model_output, model_hw_shape, origin_image)
    frame_confidences = [bbox[4] for bbox in bboxes]
    avg_confidence = np.mean(frame_confidences) if frame_confidences else 0
    confidence_buffer.append(avg_confidence)
    return list(confidence_buffer)

np.array(confidence_buffer)